...
public class UniversalWaystoneScreen extends HandledScreen<ScreenHandler> {
    ...
    private void renderWaystoneAmount(MatrixStack matrices, int x, int y) {
        this.textRenderer.draw(matrices, new TranslatableText("fwaystones.gui.displayed_waystones", this.getDiscoveredCount()), x, y, 0x161616);
    }
    ...
    protected void renderCostItem(MatrixStack matrices, int x, int y) {
        ...
        switch (config.teleportType()) {
            case "hp", "health" -> {
                ...
                text = new TranslatableText("fwaystones.cost.health");
            }
            case "hunger" -> {
                this.drawTexture(matrices, x + 4, y + 4, 177, 24, 9, 9);
                text = new TranslatableText("fwaystones.cost.hunger");
            }
            case "xp", "experience" -> {
                this.drawTexture(matrices, x + 4, y + 4, 177, 15, 9, 9);
                text = new TranslatableText("fwaystones.cost.xp");
            }
            case "level" -> {
                this.itemRenderer.renderGuiItemIcon(new ItemStack(Items.EXPERIENCE_BOTTLE), x, y);
                text = new TranslatableText("fwaystones.cost.level");
            }
            case "item" -> {
            ...
            default -> text = new TranslatableText("fwaystones.cost.free");
        }
    ...
    protected void renderForgetTooltips(MatrixStack matrixStack, int mouseX, int mouseY, int x, int y) {
        for (int i = 0; i < 5; ++i) {
            ...
            this.renderTooltip(matrixStack, new TranslatableText("fwaystones.gui.forget_tooltip"), mouseX, mouseY);
        }
    }
    ...
    protected void renderWaystoneTooltips(MatrixStack matrixStack, int mouseX, int mouseY, int x, int y, int m) {
        ...
        for (int n = this.scrollOffset; n < m && n < getDiscoveredCount(); ++n) {
            ...
            this.renderTooltip(matrixStack, new TranslatableText("fwaystones.gui.cost_tooltip", Utils.getCost(Vec3d.ofCenter(waystoneData.way_getPos()), client.player.getPos(), startDim, endDim)), mouseX, mouseY);
        }
    }
