    ...
    protected void build(FlowLayout rootComponent) {
        rootComponent.child(
                Containers.verticalFlow(Sizing.content(), Sizing.content())
                        .child(
                                Containers.verticalScroll(Sizing.fixed(120), Sizing.fixed(120),
                                        Containers.verticalFlow(Sizing.content(), Sizing.content())
                                                .configure((FlowLayout layout) -> {
                                                    var playerData = WaystonePlayerData.getData(MinecraftClient.getInstance().player);
                                                    var storage = WaystoneDataStorage.getStorage(MinecraftClient.getInstance());

                                                    if (storage == null) return;

                                                    var uuids = playerData.discoveredWaystones().stream()
                                                    ...
                                                    for (var uuid : uuids) {
                                                        ...
                                                        layout.child(
                                                                new FlowLayout(Sizing.expand(), Sizing.content(), FlowLayout.Algorithm.VERTICAL){
                                                                    @Override
