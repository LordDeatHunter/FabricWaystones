...
public class TeleportationOptimizer {
    ...
    // Configuration
    private static final int MAX_TELEPORTS_PER_TICK = 2; // Maximum teleports to process per server tick
    private static final int CHUNK_PRELOAD_RADIUS = 2; // Radius of chunks to preload around destination
    private static final int CHUNK_PRELOAD_TICKS = 300; // How long to keep chunks loaded (15 seconds)
    private static final long CHUNK_LOAD_TIMEOUT_MS = 100; // Max time to wait for chunk loading per tick

    // Teleport queue
    private final Queue<QueuedTeleport> teleportQueue = new ConcurrentLinkedQueue<>();

    // Dimension world cache to avoid iterating through all worlds repeatedly
    ...
    // Custom chunk ticket type for waystone teleportation
     ...
     * This spreads the performance cost across multiple ticks.
     */
    public void queueTeleport(ServerPlayerEntity player, ServerWorld targetWorld,
                             TeleportTarget target, Runnable onSuccess) {
        ...
        // Create preload task
        BlockPos targetPos = BlockPos.ofFloored(target.pos());
        ChunkPreloadTask preloadTask = new ChunkPreloadTask(targetWorld, targetPos);
        preloadTasks.put(playerId, preloadTask);

        // Start async chunk preloading
        asyncExecutor.submit(() -> {
            try {
                preloadChunksAsync(preloadTask, targetWorld, targetPos);
            } catch (Exception e) {
                FabricWaystones.LOGGER.warn("Error preloading chunks for waystone teleport", e);
            }
        });

        // Queue the actual teleport
        QueuedTeleport queuedTeleport = new QueuedTeleport(
            player, targetWorld, target, onSuccess, preloadTask
        );
     ...
     * Limits the number of teleports per tick to prevent MSPT spikes.
     */
    public void tick() {
        ...
        preloadTasks.entrySet().removeIf(entry ->
            System.currentTimeMillis() - entry.getValue().creationTime > 10000 // 10 second timeout
        );
    }
        ...
        for (ChunkPos chunkPos : chunksToLoad) {
            world.getServer().execute(() -> {
                try {
                    ...
                    // Try to ensure chunk is loaded
                    if (world.getChunkManager().isChunkLoaded(chunkPos.x, chunkPos.z)) {
                        task.chunksLoaded++;
                    }
                } catch (Exception e) {
                    FabricWaystones.LOGGER.warn("Error loading chunk {} for waystone teleport", chunkPos, e);
                    task.chunksLoaded++; // Count as loaded to not block forever
                }
            });
        }
    ...
    private void executeTeleport(QueuedTeleport teleport) {
        try {
            ...
            // Detach player from any vehicles
            player.detach();

            // Perform the teleport (now chunks should be loaded)
            TeleportTarget optimizedTarget = new TeleportTarget(
                teleport.targetWorld,
                teleport.target.pos(),
                teleport.target.velocity(),
                teleport.target.yaw(),
                teleport.target.pitch(),
                false // Don't add portal chunk ticket - we already loaded chunks
            );

            player.teleportTo(optimizedTarget);
            ...
            FabricWaystones.LOGGER.error("Error executing waystone teleport", e);
        }
    }
    ...
    /**
     * Cancels chunk preloading for a player.
     */
    ...
    /**
     * Gets a cached ServerWorld by dimension name, or searches for it if not cached.
     */
    ...
    /**
     * Clears the dimension cache. Should be called when worlds are unloaded.
     */
    ...
    /**
     * Shuts down the async executor. Should be called on server stop.
     */
    ...
    private static class QueuedTeleport {
        ...
        QueuedTeleport(ServerPlayerEntity player, ServerWorld targetWorld,
                      TeleportTarget target, Runnable onSuccess, ChunkPreloadTask preloadTask) {
            this.player = player;
        ...
        /**
         * Force execute after 3 seconds even if chunks aren't fully loaded
         * to prevent players from being stuck in queue forever.
         */
