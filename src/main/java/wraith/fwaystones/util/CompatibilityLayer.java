...
public class CompatibilityLayer {
    ...
    public void fromTag(NbtCompound tag) {
        if (tag.contains("Waystones")) {
            FabricWaystones.LOGGER.info("Found old save data. Converting to new save format.");
    ...
    private void parseOldTag(NbtCompound tag) {
        ...
        if (compatTags.isEmpty()) {
            FabricWaystones.LOGGER.info("No waystones found in old save data.");
            return;
        }
        HashSet<WaystoneBlockEntity> entities = new HashSet<>();
        for (int i = 0; i < compatTags.size(); ++i) {
            FabricWaystones.LOGGER.info("Converting " + i + " of " + compatTags.size() + " waystones.");
