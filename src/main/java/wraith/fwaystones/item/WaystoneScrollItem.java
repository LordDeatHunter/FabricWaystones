...
public class WaystoneScrollItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (learned > 0) {
            if (learned > 1) {
                text = new TranslatableText(
                    ...
                    new LiteralText(String.valueOf(learned)).styled(style ->
                        style.withColor(TextColor.parse(new TranslatableText("fwaystones.learned.multiple.arg_color").getString()))
                    )
                );
            } else {
                text = new TranslatableText("fwaystones.learned.single");
            }
            ...
            text = new TranslatableText("fwaystones.learned.none");
            stack.setNbt(null);
        }
        ...
        if (waystones != null) {
            tooltip.add(new TranslatableText(
                ...
                new LiteralText(String.valueOf(size)).styled(style ->
                    style.withColor(TextColor.parse(new TranslatableText("fwaystones.scroll.tooltip.arg_color").getString()))
                )
            ));
        }
    }
